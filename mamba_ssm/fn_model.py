...
class TimeModel(nn.Module):
    def __init__(self, configs):
        ...
        self.genlen = configs.genlen
        self.input_dim = configs.input_dim
        self.outputFN = nn.Sequential(
            nn.Linear(self.input_dim[1]*self.input_dim[2], self.genlen, bias=False, device=self.device, dtype=self.dtype),  
            #nn.Linear(genlen, 1, bias=False, device=device, dtype=dtype)  # Transform to [batch_size, 1]
        )
        self.tokenizer = AutoTokenizer.from_pretrained("EleutherAI/gpt-neox-20b")
        self.llm_model = MambaTimeHeadModel.from_init(self.model_name, device=self.device, dtype=self.dtype)

...
#Let's make an alternative to this LM head so that we can use this for non-language purposes
...
class MambaTimeHeadModel(nn.Module, GenerationMixin):
    ...
    def __init__(
        ...
        self.time_head = nn.Linear(d_model, output_size, bias=False, **factory_kwargs)
    ...
    def tie_weights(self):
        if self.config.tie_embeddings:
            self.time_head.weight = self.backbone.embedding.weight

    def allocate_inference_cache(self, batch_size, max_seqlen, dtype=None, **kwargs):
        return self.backbone.allocate_inference_cache(batch_size, max_seqlen, dtype=dtype, **kwargs)

    def forward(self, input_embeddings, position_ids=None, inference_params=None, num_last_tokens=0, **mixer_kwargs):
        """
        "position_ids" is just to be compatible with Transformer generation. We don't use it.
        num_last_tokens: if > 0, only return the logits for the last n tokens
        """
        ...
        CausalTimeOutput = namedtuple("CausalTimeOutput", ["logits","last_hidden_state"])
        #print("hidden states: ", hidden_states)
        #print("hidden_states[-1]", hidden_states[-1])
        return CausalTimeOutput(logits=time_logits,last_hidden_state=hidden_states)

    #maybe we can start with the pretrained version, but can still train it to behave somewhat differently?
    @classmethod
    def from_pretrained(cls, pretrained_model_name, device=None, dtype=None, **kwargs):
        config_data = load_config_hf(pretrained_model_name)
        config = MambaConfig(**config_data)
    ...
    def save_pretrained(self, save_directory):
        """
        Minimal implementation of save_pretrained for MambaTimeHeadModel.
        Save the model and its configuration file to a directory.
        """
        # Ensure save_directory exists
        os.makedirs(save_directory, exist_ok=True)

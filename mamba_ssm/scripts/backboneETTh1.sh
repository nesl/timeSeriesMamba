...
master_port_base=1086  # Base for master port calculation
batch_size=16
d_ff=128
num_params='2.7b'
llm_dim=768
...
usage() {
  echo "Usage: $0 -l <llm_layers> -e <train_epochs> -m <llm_model> -g <gpu_id>"
  exit 1
}
...
'
for train_percent in 75 50 25; do
  ...
  for seed in {1..10}; do
    ...
    echo "ETTh1 completed for pred_len=$pred_len, seq_len=$seq_len, saved to $comment"
  done
done

...
for ii in range(args.itr):
    ...
    id = 'checkpoints/llamaTest3epoch6layers'
    checkpoint = torch.load(id+".pth")
    

    model_state_dict = model.state_dict()
    #print("model original state_dict: ", model.state_dict())
    #print("model original items: ", model.items())
    for name, param in checkpoint.items():
        if name in model_state_dict:
            try:
                model_state_dict[name].copy_(param)
                print(f"Success copying parameter {name}")
            except Exception as e:
                print(f"Error copying parameter {name}: {e}")
        else:
            print(f"Parameter {name} not found in model's state_dict.")
    ...
    with torch.no_grad():
        ...
        test_loss, test_mae_loss = vali(args, accelerator, model, test_data, test_loader, criterion, mae_metric,id)
        #accelerator.print(   "Epoch: {0} | Train Loss: {1:.7f} Vali Loss: {2:.7f} Test Loss: {3:.7f} MAE Loss: {4:.7f}".format( epoch + 1, train_loss, vali_loss, test_loss, test_mae_loss))
        '''

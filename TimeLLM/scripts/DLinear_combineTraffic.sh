...
for pred_len in $((96 / downsampling_factor)) $((192 / downsampling_factor)) $((336 / downsampling_factor)) $((720 / downsampling_factor)); do
  for seed in {1..10}; do
    tag="DLinear_Traffic_${og_tag}_seq${seq_len}_pred${pred_len}_seed${seed}"
    ...
    echo "DLinear Traffic with pred_len $pred_len and seed $seed completed, saved to $comment"
  done
done

...
seq_len=$((60 / downsampling_factor))

# Define trials for different pred_len values
for pred_len in $((24 / downsampling_factor)) $((48 / downsampling_factor)) $((64 / downsampling_factor)) $((120 / downsampling_factor)); do
  for seed in {1..10}; do
    tag="DLinear_ILI_${og_tag}_seq${seq_len}_pred${pred_len}_seed${seed}"
    comment="checkpoints/${tag}"
    log_file="results/ILI/${tag}.txt"
    exec > "$log_file" 2>&1
    ...
    echo "DLinear ILI with pred_len $pred_len and seed $seed completed, saved to $comment"
  done
done

...
for pred_len in $((96 / downsampling_factor)) ; do
  for seed in {1..3}; do
    for init_seed in {11..13}; do
      tag="proportional_${heldout}_${source_type}_heldout_${og_tag}_seq${seq_len}_pred${pred_len}_seed${seed}_initseed${init_seed}"
      comment="checkpoints/${tag}"
      log_file="results/proportional/${tag}.txt"
      exec > "$log_file" 2>&1

      data_path="proportional/${source_type}_no_region.csv"
      ...
      accelerate launch --mixed_precision bf16 --num_processes $num_process --main_process_port $master_port seed_process.py \
        ...
        --model_id ${source_type}_${heldout}_heldout \
        --model $model_name \
        --data CarbonCast \
        ...
        --boundary_file "dataset/CarbonCast/proportional/${source_type}_boundaries.json" \
        --rand_init $rand_init \
        --seed $seed \
        --init_seed $init_seed \
        --save_checkpoints 1 \

...
percent=50            # Default percent is 100 (no columns removed)
col_percent=100
train_percent=100
save_checkpoints=0
...
og_tag="l${llm_layers}_d${d_model}_e${train_epochs}_m${llm_model}_n${num_params}_f${downsampling_factor}_t${percent}_c${col_percent}_tr${train_percent}_r${rand_init}"
...
for data_path in "combo1" "unit" "noisy_combo1" "noisy_unit"; do 
# Define trials for different pred_len values

  for seed in {1..3}; do
    tag="${data_path}_${og_tag}_seq${seq_len}_pred${pred_len}_seed${seed}"
    comment="checkpoints/${tag}"
    log_file="results/synth/${tag}.txt"
    exec > "$log_file" 2>&1
    ...
    echo "$data_path with pred_len $pred_len and seed $seed completed, saved to $comment"
  done
done
done

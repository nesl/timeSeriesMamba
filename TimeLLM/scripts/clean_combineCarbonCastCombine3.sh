...
#for pred_len in 336; do
  ...
  #  for init_seed in $((11)); do  
      ...
      tag="combine3_${heldout}_heldout_${og_tag}_seq${seq_len}_pred${pred_len}_seed${seed}_initseed${init_seed}"
      ...
      accelerate launch --mixed_precision bf16 --num_processes $num_process --main_process_port $master_port seed_process.py \
        ...
        --data_path combined3/combined3_clean_years_no_region.csv \
        --data_path_test ${heldout}_clean.csv \
        --model_id combined3_${heldout}_heldout_${seq_len}_${pred_len} \
        --model $model_name \
        --data CarbonCast \
        ...
        --boundary_file "dataset/CarbonCast/combined3/combined3_clean_boundaries.json" \
        --rand_init $rand_init \
        --seed $seed \
        --init_seed $init_seed \
        --save_checkpoints 1 \

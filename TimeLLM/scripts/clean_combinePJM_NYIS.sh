  ...
  for seed in {1..10}; do
    ...
    for init_seed in {11..20}; do
      ...
      accelerate launch --mixed_precision bf16 --num_processes $num_process --main_process_port $master_port seed_process.py \
        ...
        --data_path NYIS_clean.csv \
        --data_path_test PJM_clean.csv \
        --model_id NYIStrain_PJMtest_${seq_len}_${pred_len} \
        --model $model_name \
        --data CarbonCast \
      ...
      echo "PJM pretrained on NYIS with init_seed $init_seed and seed $seed completed, saved to $comment"

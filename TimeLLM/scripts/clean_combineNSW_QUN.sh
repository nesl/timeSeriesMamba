...
#for pred_len in 336; do
  for seed in {1..10}; do
    ...
    for init_seed in {11..20}; do
      tag="NSWtrain_QUNtest_${og_tag}_seq${seq_len}_pred${pred_len}_seed${seed}_initseed${init_seed}"
      comment="checkpoints/${tag}"
      log_file="results/NSWtrain_QUNtest/${tag}.txt"
      exec > "$log_file" 2>&1

      accelerate launch --mixed_precision bf16 --num_processes $num_process --main_process_port $master_port seed_process.py \
        --task_name long_term_forecast \
        --is_training 1 \
        --root_path ./dataset/aus/ \
        --data_path nsw.csv \
        --data_path_test qun.csv \
        --model_id NSWtrain_QUNtest_${seq_len}_${pred_len} \
        --model $model_name \
        --data Aus \
        --data_pretrain Aus \
        --pretrain 1 \
      ...
      echo "QUN pretrained on NSW with init_seed $init_seed and seed $seed completed, saved to $comment"

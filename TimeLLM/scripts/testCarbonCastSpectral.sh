...
for pred_len in $((96 / downsampling_factor)); do
  for seed in "${seed_array[@]}"; do
    for init_seed in "${init_seed_array[@]}"; do
      tag="spectralUniTest_${heldout}_${source_type}_heldout_${og_tag}_seq${seq_len}_pred${pred_len}_seed${seed}_initseed${init_seed}"
      checkpoint_tag="spectralUni_CISO_solar_p05_${og_tag}_seq${seq_len}_pred${pred_len}/s${seed}_i${init_seed}"
      log_file="results/spectralUniTest/${tag}.txt"
      exec > "$log_file" 2>&1
    
      data_path="spectral/heldout/${heldout}_${source_type}.csv"
      data_path_test="spectral/heldout/${heldout}_${source_type}.csv"
      ...
      echo "${heldout} heldout (${source_type}) with init_seed $init_seed and seed $seed completed"
